...
// combined with everything else sharing the same date+template window,
  ...
  for (const date of dates) {
    for (const template of templatesForDate(shiftTemplates, date)) {
      ...
      if (total > template.max_staff) {
        const note = `${total} requested/scheduled for "${template.name}" — only ${template.max_staff} allowed.`;
