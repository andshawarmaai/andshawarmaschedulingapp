...
- **One** Vercel project: `andshawarmaschedulingapp` (id `prj_myeSrvOqeHDVKRTuZNm7UdpWPHw6`) → https://andshawarmaschedulingapp.vercel.app. Earlier `andshawarma-scheduling` (no hyphen) project was a duplicate created by mistake and deleted. Do NOT create additional Vercel projects under this team without checking with the owner first — they will delete them.
- **One** GitHub repo: `andshawarmaai/andshawarmaschedulingapp`, remote name `origin`. The historical `customer` remote (`https://github.com/therayally/andshawarmatest.git`) 403s on push — the `andshawarmaai` account lacks write to `therayally`. Do NOT `git push customer`; push to `origin` only.
- **One** Neon DB; schema applied via `node db/apply-schema.mjs` (idempotent via `IF NOT EXISTS`).
- The owner's Mac (`testuser@Rays-MacBook-Air`) hosts the Hermes bridge + Cloudflare tunnel + Vercel deploy creds. The **dev box is a separate machine** that pulls from `origin` via `git pull` and runs the dev server locally — it does NOT have cloudflared, the bridge, the Vercel token, or any of the live secrets. When asked to "get this on the dev box," write the changes to the repo (commit + push to origin) — that's the only thing the dev box needs to see them.

When the user says "deploy," the canonical URL is `<project>.vercel.app` with no random suffix (e.g. `https://andshawarmaschedulingapp.vercel.app`, not `https://andshawarmaschedulingapp-<hash>-and-shawarma.vercel.app`). The hashed URL is the deployment-specific URL printed by `vercel --prod --yes` and freezes forever on first print — a user reporting "I don't see the fix" is often sitting on an old deployment-specific URL from a prior turn's output. Always ask for the address bar URL before assuming the deploy failed.

**Pre-deploy git hygiene (skipping any of these fails the push):**
- `git config user.email "andshawarmaai@protonmail.com"` — the default `your-email@example.com` placeholder is rejected by GitHub.
- No literal tokens in any committed file. Use placeholders like `$VERCEL_TOKEN`, `$CF_API_TOKEN`, `shwrm_xxxx`. GitHub's push-protection blocks any commit containing a literal `vcp_` (Vercel) or `cfat_` (Cloudflare) token string — fix by redacting the literal and replacing with the env-var name.
- Verify the build: `npm run build` must pass clean before pushing.
...
- `hermes` — POST to a local Cloudflare Tunnel URL (free, runs on the user's Mac; Mac must be on). The URL rotates on every cloudflared restart; a LaunchAgent (`com.shawarma.tunnel-watcher`) tails `/tmp/cf-tunnel.err.log` and POSTs new URLs to `/api/admin/settings/tunnel` so the app picks them up automatically. App falls back to cloud mode for the ~10s restart gap.
...
**Hermes bridge (local on the owner's Mac):** `scripts/hermes-bridge.mjs` listens on `127.0.0.1:7890`, accepts the orchestrator's payload, and writes a stub reply via `POST /api/admin/agent-chat/reply`. To enable real Hermes, replace the body of `processMessage()` with actual agent logic — the bridge script is the integration point, the Vercel orchestrator is unchanged.

**Chat attachments** (photos + PDFs + text docs only — NO audio, video, or spreadsheets, owner-confirmed scope): `POST /api/agent/chat/upload` (multipart, 10MB cap, writes to `os.tmpdir()/chat-uploads/`), `GET /api/agent/chat/attachments/[id]` (streams from disk, auth = uploader OR admin/manager, returns 410 Gone if file vanished across a Vercel cold start). The orchestrator inlines base64 (≤2MB) into the agent payload; larger files send metadata only. UI is in `src/components/AgentChatPanel.astro` with inline SVG icons only — no emoji.
...
- **Always verify the live deployment after pushing, before reporting done.** The push succeeding means GitHub has the commit; it does NOT mean Vercel has rebuilt. Pull the current production deployment's commit SHA via `GET https://api.vercel.com/v6/deployments?projectId=<id>&target=production&limit=1` and compare to your local `git rev-parse HEAD`. If they don't match, either the push didn't trigger a deploy (check Vercel's git integration) or the build is still running — wait, don't claim "deployed" until the SHAs match.
- **A wrong project ID at deploy time creates a brand new Vercel project, silently.** The `vercel deploy --prod` command reads the Vercel project ID from `.vercel/project.json`. If that file's `projectId` is stale (a previous run created a different project under a similar name), your deploy lands on the wrong project with no warning. Before deploying, sanity-check `.vercel/project.json`'s `projectName` matches the canonical project name (see the Deployment section above) and verify the project list via `GET /v9/projects` returns exactly the expected names. If you create an extra project by accident, delete it immediately (`DELETE /v9/projects/<id>`) — duplicates confuse the owner.