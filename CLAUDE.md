...
- **The Day view timeline itself also shows open slots, not just the summary cards above it**: positioned at their precise time-of-day and packed into the SAME overlap-column layout as real shift bars (`computeGhostItems`/`coverageGapBarHtml`/`dayBarsHtml` in `admin/schedule.astro`) — never a background band that real bars could fully cover. Each open slot is its own drag target/click target exactly like a coverage-panel card. Week view deliberately doesn't get these (`dayBarsHtml`'s `ghostItems` param is Day-view-only) — the narrower week columns would get too cluttered.
  - **`computeGhostItems` sizes each ghost lane's box to the EXACT sub-range still actually short — never the template's full declared window.** This bounced through three designs on 2026-09-16 before settling here, worth knowing in full since the middle one was a real dead end: (1) one ghost item per gap SEGMENT — a request overlapping just PART of a template's window split it into segments with different needed-counts each side, so the same logical "slot" could land in a different column before/after the split, fragmenting into stacked/misaligned pieces (fixed by lane-merging, below); (2) briefly LOCKED every lane's box to the template's full window regardless of which specific sub-range was actually short, after the owner watched a template's block visibly resize when an unrelated shift moved elsewhere in the day and asked to "lock in the size of the shift templates... they just keep resizing" — but this was wrong: it makes a lane's box cover portions of the window that are ALREADY covered by someone else, falsely implying more people are needed there than actually are ("it appears like there's an extra person and there isn't" — confirmed via the actual `computeDayCoverage`/`computeTemplateTimeGaps` output, not just a screenshot: the sub-ranges were correct all along). (3) reverted to sub-range precision, confirmed as the right trade-off after presenting both options directly. **A template's gap box legitimately resizing/appearing/disappearing when a DIFFERENT, unrelated shift moves elsewhere that day is correct behavior, not a bug** — any-overlap coverage means one person's shift can affect several templates' actual shortfall at once; if a future report describes this as "erratic," don't re-lock the size — the accuracy is intentional. The lane-merging itself: for lane `i`, walk the template's segments and merge every run of CONSECUTIVE segments where `needed > i` into one continuous bar before it ever reaches `packOverlaps` — so a lane needed across several adjoining segments still gets one stable box for its whole span, and only splits/ends where the real coverage genuinely changes.
  ...
  - **A coverage-gap block's label puts its own time range FIRST, template name second** (`coverageGapBarHtml`, reordered 2026-09-16) — templates are very often named after their nominal window (e.g. "9am to 9:30pm — Closer"), which reads exactly like a position claim; at Day view's narrow fixed-width columns the name alone could fill the whole ellipsis budget and cut off before ever reaching the actual range, leaving a block that LOOKED like it claimed to start at the time in its own name while being positioned somewhere completely different. Since gap boxes show the exact sub-range (see the `computeGhostItems` note above — this ISN'T always the same as the template's own window), putting the real range first (`"9:00 AM–11:30 AM · 9am to 9:30pm — Closer"`) means the part that survives truncation is always the accurate one.
