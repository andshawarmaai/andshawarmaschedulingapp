...
- **Coverage checking is instant-by-instant, not a single daily total.** `computeDayCoverage` (`client/coverage.js`) sweeps a template's window and splits it into sub-intervals wherever the concurrent headcount changes (`sweepTemplateCoverage`) — deliberately ANY-OVERLAP matching, not the strict best-fit containment `computeShiftRequestConflicts` (or the Day view timeline's `computeTemplateSlots`, below) uses (that one's answering "which named role is this shift," this one's answering "is this window covered right now, by anyone"). A shift that starts an hour early or ends an hour late still provides real coverage during the part that overlaps, and a shift that's simply time-shifted must not read as a wide-open slot. The coverage panel's "X of Y" reports the WORST moment in the window when there's a `min_staff` (so "fully staffed 9-5 but empty the last hour" correctly shows as short), and the PEAK moment for `over`/`neutral`. (`computeTemplateTimeGaps`, the function this used to share `sweepTemplateCoverage` with for the Day view timeline's old ghost boxes, was removed 2026-09-20 once the timeline moved to `computeTemplateSlots` and nothing called it any more.)
- **The Day view timeline shows ONE COLUMN PER SHIFT TEMPLATE, containing a fixed number of discrete SLOTS — not a time-sliced "how much of this window is uncovered right now" visualization any more.** (`computeTemplateSlots` in `client/coverage.js`; `templateColumnHtml`/`slotChipHtml`/`openSlotHtml`/`dayBarsHtml` in `admin/schedule.astro`.) This is the second, more fundamental redesign of this feature (2026-09-20), replacing an entire prior architecture (`computeGhostItems`/`coverageGapBarHtml`, several rounds of real back-and-forth already — see git history around 2026-09-16 for the full story if it's ever relevant) that kept a template's box resizing/reshaping/fragmenting as ANY-OVERLAP coverage from unrelated shifts changed its exact moment-by-moment shortfall — accurate, but the owner found it genuinely erratic-looking no matter which of two contradictory trade-offs it landed on ("lock the size" vs. "don't overstate need"). The fix was to stop trying to visualize time-sliced coverage in the timeline at all:
  - **A template gets exactly `max_staff ?? min_staff` slots** (skipped entirely if both are null — no defined headcount to track). Each slot is either FILLED (an approved shift or pending request that best-fits this template — `bestFitTemplate`, the same strict-containment/narrowest-match attribution `computeShiftRequestConflicts` uses, NOT the any-overlap sweep) or OPEN. Approved shifts fill slots before pending requests do; more matched rows than slots just adds extra slots beyond the base count rather than hiding anyone.
  - **The column's box is ALWAYS positioned/sized from the template's own declared `start_time`/`end_time` — full stop.** It is never derived from how much of the window happens to be covered right now, so it is structurally impossible for it to resize because some unrelated shift moved elsewhere that day. The only things that can change a column's slot COUNT are: the template's own `min_staff`/`max_staff` being edited, or the actual set of rows matching it via `bestFitTemplate` changing (which is exactly the information an admin needs to see, not an artifact to hide).
  - **A column's HEIGHT is whichever is taller: its natural time-proportional height, or enough to fit a header row (`TEMPLATE_HEADER_REM`) plus every slot at a legible minimum (`MIN_SLOT_REM`, 1.75rem — kept in sync by eye with the matching `.template-slot` CSS rule, since one is JS and the other CSS)** — a template with many slots but a short window intentionally grows taller than its time span would otherwise suggest, consistent with the owner's explicit tolerance for narrow/tall/non-time-proportional layout in exchange for never squeezing content illegible (see the Day-view-compression note below). `dayBarsHtml` tracks the tallest column's bottom edge across the whole day and returns it as `heightRem`, which `renderDay()` uses for the `.cal-day-col` container instead of the naive `(windowMin/60)*DAY_REM_PER_HOUR` — otherwise a tall column could get clipped by its own container.
  - **A FILLED slot is rendered with the exact same classes/attributes as a floating chip** (`slotChipHtml` — `.shift-bar`/`.req-bar`, `data-kind`, `data-id`, `draggable="true"`), just laid out by a flex column instead of being absolutely positioned (`.template-slot.filled { position:static; }`, a higher-specificity override declared after the base rules). This is deliberate: it means `wireContextMenus`' generic `[data-kind]` right-click, `wireShiftEditClicks`' click-to-edit, and `wireDragDrop`'s dragstart-on-`.shift-bar`/`.req-bar[draggable]` ALL pick up a slot-embedded chip automatically, with zero new wiring — dragging a name OUT of a filled slot (to reposition it, or to un-commit it back to pending via the roster row) works exactly like dragging a floating bar, because as far as those functions know, it is one.
  - **An OPEN slot (`openSlotHtml`) is a click, right-click, AND drag target — three ways to fill it** ("the admin can right-click on that column and add names," 2026-09-20). `wireCoverageInteractions` wires `.template-slot.open` into the exact same selector/handlers as `.coverage-block` (the whole-template summary card above the timeline) — click or right-click opens the Add Shift modal prefilled with the template's time; dropping a roster name does the same; dropping a pending request approves it immediately at the hours actually requested; dropping an already-approved shift opens the Edit modal prefilled with this slot's time. All of this is the SAME code path `.coverage-block` already used — only the CSS selector changed to also match `.template-slot.open`.
  - **Any shift/request that does NOT best-fit any template still gets its own floating bar**, exactly as before (`dayBarsHtml`'s `packNoReuse`/`packOverlaps` path, unchanged) — a custom, one-off shift outside every defined template's window doesn't just disappear, it renders the old way alongside the new template columns, competing for its own permanent lane like everything else in Day view.
  - **`computeDayCoverage`'s summary cards above the timeline are UNCHANGED** — still the any-overlap, instant-by-instant sweep, still reporting the WORST-moment "X of Y" against `min_staff`. That computation answers a genuinely different question ("is this window physically covered right now, by anyone") than the timeline's slots now do ("is a specific person assigned to this specific role"), and the two can legitimately disagree without either being wrong — e.g. a pending request fills a SLOT (a candidate for the role) without moving the summary card's count at all (that only counts approved shifts, see the pending-can't-close-a-gap note that used to live here — it's now really a statement about `computeDayCoverage`/`rowsForSweep` specifically, not the timeline).
- **Day view uses a separate, never-reuse packer (`packNoReuse`), NOT `packOverlaps` — Week view is the only caller of `packOverlaps` now.** `packOverlaps` frees a column the moment its occupant's `end` has passed, so a later, unrelated item can take that same x-position — exactly right for Week view, where 7 already-narrow day columns can't afford one lane per item, but in Day view it reads as one thing "stacking" underneath another even though the two never actually overlap in time (a real, explicitly-rejected-by-the-owner behavior, fixed 2026-09-16: "I don't want any column sitting on the bottom, I want them all left to right"). `packNoReuse` sorts by `(start, end)` same as `packOverlaps`, but assigns `col = index in that sorted order` and never frees/reuses a column — every unmatched floating bar AND every template column gets its own permanent lane for the whole day (see above — a template column is one `packNoReuse` item like any other). `dayBarsHtml`'s `noReuse` boolean param picks the packer and switches bars' `left`/`width` from Week's percentage split to a fixed `DAY_COL_WIDTH_REM` (7.5rem) per lane; `barHtml`/`templateColumnHtml` take pre-formatted `leftCss`/`widthCss` strings (e.g. `"37.5%"` or `"15rem"`) instead of bare percentage numbers, so either caller can plug in its own unit. The container (`.cal-day-body`, Day-view-only — Week uses `.cal-week-body`/`.cal-week-days` instead) gets `overflow-x:auto` plus a `minmax(0, 1fr)` second grid track so it doesn't just auto-grow to fit the wide `.cal-day-col`, and `renderDay()` sets that column's explicit `width`/`min-width` to `cols * DAY_COL_WIDTH_REM`rem (the `cols` count comes back from `dayBarsHtml`, which returns `{ html, cols, heightRem }`). Net effect: narrow, non-negotiable per-item columns, with horizontal scrolling picking up the slack instead of ever reusing a lane — both explicitly accepted by the owner over a more compact layout.
- **Day view also uses a much shorter vertical scale than Week — `DAY_REM_PER_HOUR` (1rem/hour) vs Week's `REM_PER_HOUR` (2.5rem/hour)** — fixed 2026-09-16 after the taller scale meant a business whose hours ran past midnight needed real vertical scrolling to see the whole day, which the owner explicitly didn't want ("I should see it all in one window view... even if the numbers are stacked on top of each other"). Only `renderDay()`'s `.cal-day-col` height uses the shorter constant (now via the `heightRem` `dayBarsHtml` returns, see above); Week is untouched (`renderWeek()` still uses `REM_PER_HOUR`). At this tighter scale a genuinely brief floating bar could shrink to an unreadable sliver, so Day-view-only bars get a `min-height` floor (`.cal-day-body .req-bar`/`.shift-bar` — `.cal-day-body` is Day-view-only, Week's `.cal-day-col` instances live under `.cal-week-days` instead; `.template-slot` has its own separate, matching floor, see above).
  - **That floor must actually be tall enough to fit its own text content, or `overflow:hidden` silently clips the BOTTOM of the letters instead of leaving room for them** — a real regression, once caught from a live report that looked exactly like a packing/positioning bug ("a compressed column... I don't remember that being a shift time"), but wasn't one at all: `getBoundingClientRect()` confirmed the boxes were correctly side-by-side with zero overlap, it was purely that a 0.6875rem/~16.5px line of text plus the element's own vertical padding needed more room than the floor first shipped at, so the bottom ~4-5px of every letter was sliced off — which reads as garbled/strikethrough text, easy to mistake for two boxes colliding. `.req-bar`/`.shift-bar` (TWO stacked lines — who/when) need `2.5rem`; a single-line element like `.template-slot` needs less (`1.75rem`). **If any such floor is ever changed again, verify with `getBoundingClientRect()` that the text's own bottom edge is `<=` the container's bottom edge — don't just eyeball a screenshot, this exact bug looks identical to an actual layout collision at a glance.**
  ...
  - **Dragging an approved shift back OUT onto the Day view's roster-name row un-commits it to pending** — the reverse of dragging a name IN to fill a slot, added 2026-09-16 ("Is there no way to drag a staff member out of the shift box... put it back to pending status?"). `wireCoverageInteractions` wires `.coverage-roster` itself as a drop target for `kind === 'shift-approved'`: on drop (after a `confirm()`), `POST /api/shifts/:id/revert-to-pending` calls `db.revertShiftToPending(shiftId)` (in both `local.js` and `neon.js`), which creates a fresh `shift_requests` row (status `pending`, action `create`, same user/date/time/notes) and deletes the `shifts` row (plus any swap post referencing it). Works uniformly whether the shift originally came from an approved request (whose own `shift_requests` row stays around with status `approved` — stale history, nothing reads it) or was added directly by an admin, or now, whether it's a floating bar or a slot-embedded chip (see above — same markup, same wiring). Deliberately bypasses tier-limit checks — this isn't a new commitment being requested, it's an existing one being undone. Not undo-tracked, consistent with approve/deny being out of Undo's scope. **Needs a shift template applying to that date for the roster row to even render** (`renderCoveragePanel` returns nothing when there's no applicable template).
...
- **Filling a coverage-block/open-slot has THREE independent paths, and every one of them either opens the Add Shift modal pre-filled or approves-in-place, never a silent instant save**: drag a roster name onto it; click a name to arm it (`.roster-chip.selected`) then click the target; or right-click the target directly (added 2026-09-20 alongside the template-slot redesign above). All three work identically whether the target is a `.coverage-block` (the whole-template summary card) or a `.template-slot.open` (an individual slot in the timeline) — same selector, same handlers. `selectedRosterUserId` in `admin/schedule.astro` tracks the click-armed name; it's cleared on every render and after each use. A FOURTH interaction, dropping an existing PENDING REQUEST onto either target, approves it immediately (`PATCH /api/shift-requests/:id { status: 'approved' }`) rather than opening a modal, since the point is visually confirming "yes, this person for this slot" for a request that's often already sitting right next to the target — and critically, at the hours the staff member actually asked for, never stretched to fill the whole target. Not undo-tracked, consistent with the rest of approve/deny being out of undo's scope.
- **A shift template's `min_staff ?? max_staff` (whichever is used, per computeTemplateSlots above) is what actually drives every "needs coverage" signal** — the coverage panel's under/filled status, and now the Day view timeline's slot count, all of it. If BOTH are null there's no coverage signal at all: no colored status, no slot column. When staffing numbers from a customer read like "1 Person"/"3 Person" (a fixed target, not just a ceiling), set BOTH `min_staff` and `max_staff` to that number — setting only one or the other (an early mistake on the demo data) can make the feature behave unexpectedly (e.g. `max_staff` alone with the coverage panel produces a `neutral` status even on an empty day, since there's no minimum to be short of).
