...
- **The Day view timeline itself also shows open slots, not just the summary cards above it**: positioned at their precise time-of-day and packed into the SAME overlap-column layout as real shift bars (`computeGhostItems`/`coverageGapBarHtml`/`dayBarsHtml` in `admin/schedule.astro`) — never a background band that real bars could fully cover. Each open slot is its own drag target/click target exactly like a coverage-panel card. Week view deliberately doesn't get these (`dayBarsHtml`'s `ghostItems` param is Day-view-only) — the narrower week columns would get too cluttered.
  - **`computeGhostItems` assigns each template a fixed set of LANE indexes (0..maxNeeded-1), independent of how the sweep segments the window — not one ghost item per gap segment.** An earlier version created `needed` separate ghost items per segment, and a real request overlapping just PART of a template's window (e.g. someone's 11am-7pm shift inside a 9am-6pm/min-3 block) splits that block into segments with a different needed-count each side of 11am — handing the packer several independent short ghosts per segment let it column-assign each segment on its own, so the same logical "slot" could land in a different column before and after the split, visually fragmenting into stacked/misaligned pieces instead of clean vertical lanes (a real bug, caught and fixed 2026-09-16). The fix: for lane `i`, walk the template's segments and merge every run of CONSECUTIVE segments where `needed > i` into one continuous bar before it ever reaches `packOverlaps` — so a lane needed across several segments stays one stable column for its whole span, and only actually shrinks/ends where the real coverage genuinely changes.
  ...
  - **A coverage-gap block's label puts its own time range FIRST, template name second** (`coverageGapBarHtml`, reordered 2026-09-16) — templates are very often named after their nominal window (e.g. "9am to 9:30pm — Closer"), which reads exactly like a position claim; at Day view's narrow fixed-width columns the name alone could fill the whole ellipsis budget and cut off before ever reaching the actual range, leaving a block that LOOKED like it claimed to start at the time in its own name while being positioned somewhere completely different (a real reported confusion: "a column that says 9 AM to 9:30 PM... doesn't show that it starts at 9 AM"). Putting the real range first (`"7:00 PM–9:30 PM · 9am to 9:30pm — Closer"`) means the part that survives truncation is always the accurate one.
  - **The "Time off" row now has its own soft background + a heavier bottom border**, rather than just a thin 1px line — it sits directly above the hour rail (`.cal-hours`) in `.cal-day-body`, both using the same 3.5rem left-column width, and without a strong visual break it read as if "Time off" were labeling the hour numbers below it (the general shift-time axis) rather than its own, often-empty row of time-off entries (fixed 2026-09-16, a real reported confusion: "that word time off is not correct. This is the shift times").
