...
- **`scripts/hermes-bridge.mjs` now has a local-fine-tuned-model fallback path, added 2026-09-21 when Hermes ran out of MiniMax credits mid-session and had no way to serve a reply at all.** `LOCAL_MODEL_URL` points at an OpenAI-compatible chat-completions endpoint (e.g. `mlx_lm.server --model training-data/fused-model --port 8081`, once training-data/README.md's fine-tune has actually been run and fused) serving the model fine-tuned from `training-data/`. `LOCAL_MODEL_MODE` controls routing: `fallback` (default once `LOCAL_MODEL_URL` is set — try Hermes, use the local model only if Hermes errors), `primary` (local first, Hermes as the fallback), `only` (skip Hermes entirely — the mode to use while credits are out), `off` (default when unset). The local path makes zero outbound API calls per message, so it keeps working regardless of MiniMax credit state. `runLocalModel()` sends `{messages, tools}` in the same shape `training-data/convert_to_mlx.mjs` trained the model on (see that file's `TOOLS` constant — `LOCAL_MODEL_TOOLS` in the bridge mirrors it **by hand**, keep both in sync with `scripts/mcp-server.mjs`'s `registerTool` calls if a tool changes), parses whichever of structured `tool_calls` or scraped `<tool_call>{...}</tool_call>` text tags the server actually returned (varies by MLX server version — both are handled), and converts each into the same `{method, endpoint, body}` action shape the legacy Hermes JSON-block path already produced, so the Vercel orchestrator's `executeAction()` needs no changes to run either path's output. Unlike the Hermes+MCP path, the local model never executes anything itself — `mcp_executed` is always `false` here, so the orchestrator always runs `actions`. Verified end-to-end against a fake local-model HTTP server standing in for `mlx_lm.server` (structured `tool_calls`, tag-scraping fallback, and the Hermes-fails-then-falls-back-to-local `fallback` mode all produced the correct `actions` array) — not yet verified against a REAL fine-tuned model, since no training run has actually happened yet (see `training-data/README.md`'s "Not yet done").
