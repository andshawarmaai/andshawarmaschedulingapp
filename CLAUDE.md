...
- **The Day view timeline itself also shows open slots, not just the summary cards above it**: an understaffed template contributes one dashed red "{name} — open slot" block per still-needed head, positioned at that template's actual time-of-day and packed into the SAME overlap-column layout as real shift bars (`computeGhostItems`/`coverageGapBarHtml`/`dayBarsHtml` in `admin/schedule.astro`) — never a background band that real bars could fully cover. Each open slot is its own drag target/click target exactly like a coverage-panel card. Week view deliberately doesn't get these (`dayBarsHtml`'s `ghostItems` param is Day-view-only) — the narrower week columns would get too cluttered.
